...
function AddSamples(props) {
  ...
  const handleAddSample = (accordionData, indexId) => {
    console.log(indexId)
    const data = {sampleNumber: accordionData.sampleNumber, description: accordionData.description}
    const currentSample = sampleArray[indexId]
    if (currentSample) {
      const newSample = {...currentSample, ...data}
      const updatedSamples = sampleArray.map((sample, index) => indexId == index ? newSample : sample) 
  ...
  const handleAddTests = (sampleIndex, { test_id }) => {
    ...
    setSampleArray(sampleArray.map((sample, index) => index == sampleIndex ? currentSample : sample))
  ...
  const handleSubmit = () => {
    ...
    const json = sampleArray.map((sample) => ({...sample, client_id: client.id, chain_of_custody: chain}))
    console.log(json)
